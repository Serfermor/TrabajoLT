# -*- coding: utf-8 -*-
"""
Created on Mon Jan  4 16:26:50 2021
...
data_v = np.load('Subjectivity.npy')
...
plt.savefig('LSTM_Plots\Subjectivity_DB.eps')
...
# Entrenamiento de la red neuronal 'LSTM': MODELO 1
...
plt.grid(True)
plt.title('DNN LSTM: Funcion de perdidas - Subjectivity ')
plt.xlabel('EPOCH')
plt.ylabel('Mean Square Error Loss')

# Guardamos la imagen como png, para su posterior estudio
plt.savefig('LSTM_Plots\Subjectivity_Loss_F.eps')
...
horas=np.arange(2,pred_samples*2+2,2)
...
# Guardamos la imagen como png, para su posterior estudio
plt.savefig('LSTM_Plots\Subjectivity_Testing.eps')
...
# Guardamos la imagen como png, para su posterior estudio
plt.savefig('LSTM_Plots\Subjectivity_Forecast.eps')

plt.show()

# Guardamos los datos obtenidos
np.save("DB_Predicciones/Subjectivity"+"_"+str(pred_samples)+"_Predicted"+".npy", data_predicted)